...
export class ObjectExplorerSidebarService {
  ...
  public saveColumnName(options) {
    let serviceUrl = `${
      environment.baseUrl
    }semantic_layer/table_column_rename/`;
  ...
  public getReports(tableId) {
    let getReportsUrl = `${
      environment.baseUrl
    }semantic_layer/dependent_reports/?sl_tables_id=${tableId}`;

    return this.http.get(getReportsUrl).pipe(catchError(this.handleError));
  }
  ...
  public deleteTable(tableId) {
    ...
    return this.http.post(deleteUrl, data).pipe(catchError(this.handleError));
  }
}
